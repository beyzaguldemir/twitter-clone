...
const Main = ({user}) => {
  const [tweets,setTweets]=useState(null)
    useEffect(()=>{
      ...
      const unsub=onSnapshot(q,(snapshot)=>{
        // tweetlerin geçici olarak tutulacagı dizi
        const temp=[]
        // doc'ların içerisindeki veriye erisip gecici diziye aktar
  ...
  return (
    <main className='border border-zinc-600 overflow-y-auto'>
      ...
      {!tweets ? <div className='flex justify-center my-20 scale-[1.5]'><Loader/></div> : tweets.map((tweet)=> <Post tweet={tweet} key={tweet.id}/>)}
    </main>
  )
}
